...
class MemberShiftRequestView(APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, member_id):
        ...
        Both sender and reciver must be in the same workspace as the shifts. Member id must be url parameter. Accepted body fields:
        ...
        # get sender member
        try:
            sender = WorkspaceMember.objects.get(pk=member_id)
        except WorkspaceMember.DoesNotExist:
            response["error"]["message"] = "Sender does not exist."
            return Response(response, status=status.HTTP_404_NOT_FOUND)

        # ensure sender is user
        if sender.user.id != request.user.id:
            response["error"][
                "message"
            ] = "You do not have permission to make a request for this member."
            return Response(response, status=status.HTTP_403_FORBIDDEN)
        ...
        if sender_shift.open == True or sender_shift.member != sender:
            ...
            return Response(response, status=status.HTTP_400_BAD_REQUEST)
        if recipient_shift_present and (
            recipient_shift.open == True or recipient_shift.member != recipient
        ):
